...
def get_one_page_ppt_url_name_dict(urlStr):
    ...
    for a_tag in a_tag_list:
        ...
        if img:
            ...
            if url_path.count('/')==2:
                print(url_path.split('/')[1],utils.get_chinese(img.get("alt")),url_path)
...
class PptUrlInfo(object):
    def __init__(self,typeStr,name,detailsPath):
...
for pageUrl in ppt_51_url_list:
    url_name_dict = {}
    ...
    for a_tag in a_tag_list:
        ...
        if img:
            url_path = a_tag.get("href")
            if url_path.count('/')==2:
                pptUrlInfoList.append(PptUrlInfo(url_path.split('/')[1],utils.get_chinese(img.get("alt")),url_path))
                print(url_path.split('/')[1],utils.get_chinese(img.get("alt")),url_path)

print(len(pptUrlInfoList))
sqliteUtil.insertPptUrlInfoList(pptUrlInfoList)
